...
public final class EmeraldWhipHitEffects {
    ...
    private static final float HIT_SIZE_SCALE = 0.25F;
    private static final float HIT_ALPHA_SCALE = 1.00F;

    private EmeraldWhipHitEffects() {
    }
    ...
    private static void drawExplosion(
        ...
        drawBillboardDiamond(vertices, pose, cameraPos, center,
                (glowPass ? 0.56F : 0.31F) * (0.70F + pop * 0.42F) * HIT_SIZE_SCALE,
                glowPass ? 196 : 132,
                glowPass ? 255 : 245,
                glowPass ? 214 : 168,
                Math.round((glowPass ? 92.0F : 212.0F)
                        * (1.0F - smoothstep(0.02F, 0.38F, life)) * HIT_ALPHA_SCALE),
                event.seed * 0.00011F + life * 0.35F);

        drawComicBurstRing(vertices, pose, cameraPos, center, basis,
                (0.35F + pop * 0.92F) * HIT_SIZE_SCALE,
                (glowPass ? 0.15F : 0.055F) * HIT_SIZE_SCALE,
                event.seed,
                glowPass ? 74 : 24,
                glowPass ? 255 : 108,
                glowPass ? 132 : 54,
                Math.round((glowPass ? 48.0F : 220.0F) * fade * HIT_ALPHA_SCALE));
        ...
        for (int i = 0; i < longNeedles; i++) {
            ...
            float startRadius = (0.74F + random.nextFloat() * 0.18F) * HIT_SIZE_SCALE;
            float length = (0.86F + random.nextFloat() * 0.92F) * (0.70F + pop * 0.42F) * HIT_SIZE_SCALE;
            float width = (glowPass ? 0.074F : 0.026F) * HIT_SIZE_SCALE;
            drawComicNeedle(vertices, pose, cameraPos, center, basis, angle,
                    startRadius, length, width,
                    glowPass ? 92 : 28,
                    glowPass ? 255 : 126,
                    glowPass ? 150 : 62,
                    Math.round((glowPass ? 40.0F : 210.0F) * fade * HIT_ALPHA_SCALE));
        }

        int accents = 7;
        for (int i = 0; i < accents; i++) {
            float angle = (float) (random.nextDouble() * Math.PI * 2.0D);
            float startRadius = (0.72F + random.nextFloat() * 0.34F) * HIT_SIZE_SCALE;
            float length = (0.16F + random.nextFloat() * 0.25F) * HIT_SIZE_SCALE;
            drawComicNeedle(vertices, pose, cameraPos, center, basis, angle,
                    startRadius, length,
                    (glowPass ? 0.045F : 0.016F) * HIT_SIZE_SCALE,
                    glowPass ? 112 : 34,
                    glowPass ? 255 : 144,
                    glowPass ? 166 : 74,
                    Math.round((glowPass ? 28.0F : 164.0F) * fade * HIT_ALPHA_SCALE));
        }
    }
