...
public final class AmethystWhipHitEffects {
    ...
    private AmethystWhipHitEffects() {
    }
    ...
    private static void drawExplosion(
        ...
        float age = now - event.spawnTick;
        float life = Mth.clamp(age / 5.5F, 0.0F, 1.0F);
        float appear = smoothstep(0.0F, 0.08F, life);
        ...
        BillboardBasis basis = trajectoryBasis(center, cameraPos, event.to.subtract(event.from));
        float length = (glowPass ? 5.70F : 4.64F) * HIT_SIZE_SCALE * (0.96F + life * 0.08F);
        float slant = (glowPass ? 0.12F : 0.10F) * (event.magicMirror ? -1.0F : 1.0F);
        float outerHalfWidth = (glowPass ? 0.09F : 0.065F) * HIT_SIZE_SCALE;
        float coreHalfWidth = outerHalfWidth * 0.46F;
        int outerR = event.magicMirror ? (glowPass ? 182 : 156) : (glowPass ? 124 : 104);
        int outerG = event.magicMirror ? (glowPass ? 134 : 110) : (glowPass ? 86 : 72);
        int outerB = event.magicMirror ? (glowPass ? 255 : 236) : (glowPass ? 236 : 212);
        int coreR = event.magicMirror ? 234 : 222;
        int coreG = event.magicMirror ? 214 : 198;
        int coreB = 255;

        drawPointedSlash(vertices, pose, cameraPos, center, basis,
                length, slant, outerHalfWidth,
                outerR, outerG, outerB,
                Math.round((glowPass ? 92.0F : 238.0F) * energy));
        drawPointedSlash(vertices, pose, cameraPos, center, basis,
                length * 0.92F, slant * 0.85F, coreHalfWidth,
                coreR, coreG, coreB,
                Math.round((glowPass ? 104.0F : 252.0F) * energy));
    }
