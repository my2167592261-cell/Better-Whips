...
public final class RoyalSlimeWhipItemRenderer extends BlockEntityWithoutLevelRenderer {
    private static final ResourceLocation TEXTURE = ResourceLocation.fromNamespaceAndPath(
            BetterWhipsMod.MOD_ID, "textures/item/royal_slime_whip.png");
    private static final RenderType RENDER_TYPE = RenderType.entityTranslucent(TEXTURE);

    public RoyalSlimeWhipItemRenderer(BlockEntityRenderDispatcher blockEntityRenderDispatcher,
    ...
    public void renderByItem(ItemStack stack, ItemDisplayContext context, PoseStack poseStack,
        ...
        if (context == ItemDisplayContext.GUI) {
            WhipGuiFastRenderer.render(
                    WhipGuiFastRenderer.Style.ROYAL_SLIME, poseStack, bufferSource, packedOverlay);
            return;
        }
        ...
        RoyalSlimeWhipPhysics.RenderContext renderContext =
                RoyalSlimeWhipPhysics.currentRenderContext();
        if (held && renderContext != null) {

            RoyalSlimeWhipPhysics.captureRenderedSocket(poseStack, renderContext);
        }

        VertexConsumer consumer = bufferSource.getBuffer(RENDER_TYPE);
        RoyalSlimeWhipGeometry.INSTANCE.renderHandle(
                poseStack, consumer, packedLight, packedOverlay);
        if (context.firstPerson() && renderContext != null) {

            RoyalSlimeWhipPhysics.renderFirstPersonHandLash(
                    poseStack, renderContext, consumer, packedOverlay);
        }
        if (!held) {

            RoyalSlimeWhipGeometry.INSTANCE.renderStoredCoil(
                    poseStack, consumer, packedLight, packedOverlay);
        }
        poseStack.popPose();
    }
