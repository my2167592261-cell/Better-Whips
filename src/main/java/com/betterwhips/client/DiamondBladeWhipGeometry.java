...
final class DiamondBladeWhipGeometry {
    static final int SEGMENT_COUNT = DiamondBladeWhipDimensions.SEGMENT_COUNT;
    ...
    static final float[] AUTHORED_PIVOT_Z = DiamondBladeWhipDimensions.authoredPivotZFloatCopy();
    static final float[] REST_LENGTHS = buildRestLengths();

    static final double[] SEGMENT_COLLIDER_RADIUS = {
            0.07700D, 0.07600D, 0.07500D, 0.07400D, 0.07300D, 0.07200D, 0.07100D, 0.07000D, 0.06900D, 0.06800D, 0.06700D, 0.06600D, 0.06500D, 0.06400D, 0.06300D, 0.06200D, 0.06100D, 0.06000D, 0.05900D, 0.05800D, 0.05700D, 0.05600D, 0.05500D, 0.05400D
    };

    private static final String MODEL_RESOURCE =
            "/assets/better_whips/models/entity/diamond_blade_whip.bbmodel";
    static final DiamondBladeWhipGeometry INSTANCE = load();

    private final List<CubeGeometry> handle;
    private final List<List<CubeGeometry>> segments;
    private final List<CubeGeometry> tip;

    private DiamondBladeWhipGeometry(List<CubeGeometry> handle,
    ...
    private static DiamondBladeWhipGeometry load() {
        try (var stream = DiamondBladeWhipGeometry.class.getResourceAsStream(MODEL_RESOURCE)) {
            if (stream == null) {
                throw new IllegalStateException("Missing Diamond Blade Whip model: " + MODEL_RESOURCE);
            }
            ...
            return new DiamondBladeWhipGeometry(List.copyOf(handle), List.copyOf(immutableSegments), List.copyOf(tip));
        } catch (Exception exception) {
            throw new IllegalStateException("Failed to load Diamond Blade Whip Blockbench geometry",
                    exception);
        }
    }
