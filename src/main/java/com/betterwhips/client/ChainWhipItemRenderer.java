...
public final class ChainWhipItemRenderer extends BlockEntityWithoutLevelRenderer {
    private static final ResourceLocation TEXTURE = ResourceLocation.fromNamespaceAndPath(
            BetterWhipsMod.MOD_ID, "textures/item/chain_whip.png");
    private static final RenderType RENDER_TYPE = RenderType.entityTranslucent(TEXTURE);

    public ChainWhipItemRenderer(BlockEntityRenderDispatcher blockEntityRenderDispatcher,
    ...
    public void renderByItem(ItemStack stack, ItemDisplayContext context, PoseStack poseStack,
        ...
        if (context == ItemDisplayContext.GUI) {
            WhipGuiFastRenderer.render(
                    WhipGuiFastRenderer.Style.CHAIN, poseStack, bufferSource, packedOverlay);
            return;
        }
        ...
        ChainWhipPhysics.RenderContext renderContext =
                ChainWhipPhysics.currentRenderContext();
        if (held && renderContext != null) {

            ChainWhipPhysics.captureRenderedSocket(poseStack, renderContext);
        }

        VertexConsumer consumer = bufferSource.getBuffer(RENDER_TYPE);
        ChainWhipGeometry.INSTANCE.renderHandle(
                poseStack, consumer, packedLight, packedOverlay);
        if (context.firstPerson() && renderContext != null) {

            ChainWhipPhysics.renderFirstPersonHandLash(
                    poseStack, renderContext, consumer, packedOverlay);
        }
        if (!held) {

            ChainWhipGeometry.INSTANCE.renderStoredCoil(
                    poseStack, consumer, packedLight, packedOverlay);
        }
        poseStack.popPose();
    }
