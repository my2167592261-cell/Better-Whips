...
final class EmeraldWhipGeometry {
    static final int SEGMENT_COUNT = EmeraldWhipDimensions.SEGMENT_COUNT;
    ...
    static final float[] AUTHORED_PIVOT_Z = EmeraldWhipDimensions.authoredPivotZFloatCopy();
    ...
    private static final String MODEL_RESOURCE =
            "/assets/better_whips/models/entity/emerald_whip.bbmodel";
    static final EmeraldWhipGeometry INSTANCE = load();

    private final List<CubeGeometry> handle;
    private final List<List<CubeGeometry>> segments;
    private final List<CubeGeometry> tip;

    private EmeraldWhipGeometry(List<CubeGeometry> handle,
    ...
    private static EmeraldWhipGeometry load() {
        try (var stream = EmeraldWhipGeometry.class.getResourceAsStream(MODEL_RESOURCE)) {
            if (stream == null) {
                throw new IllegalStateException("Missing Emerald Whip model: " + MODEL_RESOURCE);
            }
            ...
            return new EmeraldWhipGeometry(List.copyOf(handle), List.copyOf(immutableSegments), List.copyOf(tip));
        } catch (Exception exception) {
            throw new IllegalStateException("Failed to load Emerald Whip Blockbench geometry",
                    exception);
        }
    }
