...
final class TrainerWhipGeometry {
    static final int SEGMENT_COUNT = TrainerWhipDimensions.SEGMENT_COUNT;
    static final float MODEL_UNIT = 1.0F / 16.0F;
    static final float TEXTURE_SIZE = 64.0F;

    private static final float RELIEF_TEXTURE_U_OFFSET = 0.0F;
    ...
    static final float[] AUTHORED_PIVOT_Z = TrainerWhipDimensions.authoredPivotZFloatCopy();
    ...
    private static final String MODEL_RESOURCE =
            "/assets/better_whips/models/entity/trainer_whip.bbmodel";
    static final TrainerWhipGeometry INSTANCE = load();

    private final List<CubeGeometry> handle;
    private final List<List<CubeGeometry>> segments;
    private final List<CubeGeometry> tip;

    private TrainerWhipGeometry(List<CubeGeometry> handle,
    ...
    private static TrainerWhipGeometry load() {
        try (var stream = TrainerWhipGeometry.class.getResourceAsStream(MODEL_RESOURCE)) {
            if (stream == null) {
                throw new IllegalStateException("Missing Trainer Whip model: " + MODEL_RESOURCE);
            }
            ...
            return new TrainerWhipGeometry(List.copyOf(handle),
                    List.copyOf(immutableSegments), List.copyOf(tip));
        } catch (Exception exception) {
            throw new IllegalStateException("Failed to load Trainer Whip Blockbench geometry",
                    exception);
        }
    }
