...
public final class TrainerWhipItemRenderer extends BlockEntityWithoutLevelRenderer {
    private static final ResourceLocation TEXTURE = ResourceLocation.fromNamespaceAndPath(
            BetterWhipsMod.MOD_ID, "textures/item/trainer_whip.png");
    private static final RenderType RENDER_TYPE = RenderType.entityTranslucent(TEXTURE);

    public TrainerWhipItemRenderer(BlockEntityRenderDispatcher blockEntityRenderDispatcher,
    ...
    public void renderByItem(ItemStack stack, ItemDisplayContext context, PoseStack poseStack,
        ...
        if (context == ItemDisplayContext.GUI) {
            WhipGuiFastRenderer.render(
                    WhipGuiFastRenderer.Style.TRAINER, poseStack, bufferSource, packedOverlay);
            return;
        }
        ...
        TrainerWhipPhysics.RenderContext renderContext =
                TrainerWhipPhysics.currentRenderContext();
        if (held && renderContext != null) {

            TrainerWhipPhysics.captureRenderedSocket(poseStack, renderContext);
        }

        VertexConsumer consumer = bufferSource.getBuffer(RENDER_TYPE);
        TrainerWhipGeometry.INSTANCE.renderHandle(
                poseStack, consumer, packedLight, packedOverlay);
        if (context.firstPerson() && renderContext != null) {

            TrainerWhipPhysics.renderFirstPersonHandLash(
                    poseStack, renderContext, consumer, packedOverlay);
        }
        if (!held) {

            TrainerWhipGeometry.INSTANCE.renderStoredCoil(
                    poseStack, consumer, packedLight, packedOverlay);
        }
        poseStack.popPose();
    }
