...
public final class GoldenHeavyWhipItemRenderer extends BlockEntityWithoutLevelRenderer {
    private static final ResourceLocation TEXTURE = ResourceLocation.fromNamespaceAndPath(
            BetterWhipsMod.MOD_ID, "textures/item/golden_heavy_whip.png");
    private static final RenderType RENDER_TYPE = RenderType.entityTranslucent(TEXTURE);

    public GoldenHeavyWhipItemRenderer(BlockEntityRenderDispatcher blockEntityRenderDispatcher,
    ...
    public void renderByItem(ItemStack stack, ItemDisplayContext context, PoseStack poseStack,
        ...
        if (context == ItemDisplayContext.GUI) {
            WhipGuiFastRenderer.render(
                    WhipGuiFastRenderer.Style.GOLDEN, poseStack, bufferSource, packedOverlay);
            return;
        }
        ...
        GoldenHeavyWhipPhysics.RenderContext renderContext =
                GoldenHeavyWhipPhysics.currentRenderContext();
        if (held && renderContext != null) {

            GoldenHeavyWhipPhysics.captureRenderedSocket(poseStack, renderContext);
        }

        VertexConsumer consumer = bufferSource.getBuffer(RENDER_TYPE);
        GoldenHeavyWhipGeometry.INSTANCE.renderHandle(
                poseStack, consumer, packedLight, packedOverlay);
        if (context.firstPerson() && renderContext != null) {

            GoldenHeavyWhipPhysics.renderFirstPersonHandLash(
                    poseStack, renderContext, consumer, packedOverlay);
        }
        if (!held) {

            GoldenHeavyWhipGeometry.INSTANCE.renderStoredCoil(
                    poseStack, consumer, packedLight, packedOverlay);
        }
        poseStack.popPose();
    }
