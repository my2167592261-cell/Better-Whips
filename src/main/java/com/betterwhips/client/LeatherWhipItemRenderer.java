...
public final class LeatherWhipItemRenderer extends BlockEntityWithoutLevelRenderer {
    private static final ResourceLocation TEXTURE = ResourceLocation.fromNamespaceAndPath(
            BetterWhipsMod.MOD_ID, "textures/item/leather_whip.png");
    private static final RenderType RENDER_TYPE = RenderType.entityTranslucent(TEXTURE);

    public LeatherWhipItemRenderer(BlockEntityRenderDispatcher blockEntityRenderDispatcher,
    ...
    public void renderByItem(ItemStack stack, ItemDisplayContext context, PoseStack poseStack,
        ...
        if (context == ItemDisplayContext.GUI) {
            WhipGuiFastRenderer.render(
                    WhipGuiFastRenderer.Style.LEATHER, poseStack, bufferSource, packedOverlay);
            return;
        }
        ...
        LeatherWhipPhysics.RenderContext renderContext =
                LeatherWhipPhysics.currentRenderContext();
        if (held && renderContext != null) {

            LeatherWhipPhysics.captureRenderedSocket(poseStack, renderContext);
        }

        VertexConsumer consumer = bufferSource.getBuffer(RENDER_TYPE);
        LeatherWhipGeometry.INSTANCE.renderHandle(
                poseStack, consumer, packedLight, packedOverlay);
        if (context.firstPerson() && renderContext != null) {

            LeatherWhipPhysics.renderFirstPersonHandLash(
                    poseStack, renderContext, consumer, packedOverlay);
        }
        if (!held) {

            LeatherWhipGeometry.INSTANCE.renderStoredCoil(
                    poseStack, consumer, packedLight, packedOverlay);
        }
        poseStack.popPose();
    }
